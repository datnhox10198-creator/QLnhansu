...
export default function AdminDashboard() {
  ...
  const today = new Intl.DateTimeFormat(locale, { weekday: 'long', day: '2-digit', month: 'long' }).format(new Date());
  ...
  return (
    <div className="space-y-6">
      <section className="dashboard-hero">
        <div className="relative z-10 max-w-2xl">
          ...
          <h2 className="mt-3 text-3xl font-bold tracking-[-0.035em] text-slate-800 sm:text-4xl">
            {t('Tổng quan hôm nay.')} <span className="hero-highlight">{t('Có gì cần chú ý?')}</span>
          </h2>
          <p className="mt-3 max-w-xl text-sm leading-6 text-slate-600">
            {t('Một cái nhìn nhanh về đội ngũ, lịch nghỉ và những việc cần bạn xử lý.')}
          </p>
          <div className="mt-7 flex flex-wrap gap-3">
            <Link to="/employees" className="btn hero-primary">
              {t('Xem đội ngũ')} <ArrowUpRight size={16} />
            </Link>
            <Link to="/reports" className="btn hero-secondary">
              <Flame size={16} /> {t('Xem báo cáo')}
            </Link>
          </div>
        </div>
        <div className="hero-metric relative z-10">
          ...
          <p className="mt-4 text-center text-xs font-semibold text-slate-600">{stats.pendingLeaves ?? 0} {t('request đang chờ bạn')}</p>
        </div>
      </section>
      ...
      <LeaveCalendar title={t('Lịch nghỉ phép toàn công ty')} />
    </div>
  );
}
