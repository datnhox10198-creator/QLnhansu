    ...
    <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="flex flex-col gap-3 border-b border-slate-200 px-5 py-4 md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="grid grid-cols-2 rounded-md border border-slate-200 bg-slate-50 p-1 text-sm">
            <button className={`rounded px-3 py-1.5 font-semibold ${mode === 'week' ? 'bg-white text-brand shadow-sm' : 'text-slate-500'}`} onClick={() => setMode('week')}>Tuần</button>
            <button className={`rounded px-3 py-1.5 font-semibold ${mode === 'month' ? 'bg-white text-brand shadow-sm' : 'text-slate-500'}`} onClick={() => setMode('month')}>Tháng</button>
          </div>
      ...
      <div className={`grid grid-cols-7 ${mode === 'week' ? 'min-h-72' : ''}`}>
        {days.map((day) => {
          ...
          return (
            <div key={day.toISOString()} className={`min-h-28 border-b border-r border-slate-100 p-2 ${outsideMonth ? 'bg-slate-50 text-slate-400' : 'bg-white'} ${today ? 'ring-2 ring-inset ring-teal-200' : ''}`}>
              <div className="mb-2 flex items-center justify-between">
                <span className={`grid h-7 w-7 place-items-center rounded-md text-sm font-bold ${today ? 'bg-brand text-white' : 'text-slate-700'}`}>
                  {day.getDate()}
                </span>
              </div>
              <div className="space-y-1">
                {dayLeaves.slice(0, mode === 'week' ? 4 : 2).map((leave) => (
                  <div key={leave._id} className={`truncate rounded-md px-2 py-1 text-xs font-semibold ring-1 ${statusClass[leave.status] || statusClass.Pending}`}>
                    {leave.employeeId?.fullName || 'Nhân viên'} - {leave.status}
                  </div>
                ))}
